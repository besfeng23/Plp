...
const PAYMENT_STATUS_LABELS = {
  ...
  UNMATCHED_REFERENCE: 'Unmatched reference — no booking matches the provider reference',
  UNMATCHED_PAYMENT: 'Unmatched payment — booking exists but no matching payment row exists',
  ORDER_ID_MISMATCH: 'Order mismatch — captured PayPal order differs from stored session',
  ORDER_SESSION_MISSING: 'Order session missing — stored PayPal order/session is blank',
  ORDER_ID_MISSING: 'Order ID missing — provider response did not include an order id',
  DATABASE_UNCONFIGURED: 'Database unconfigured — payment cannot be verified safely',
  PAYPAL_NOT_CONFIGURED: 'PayPal not configured — checkout cannot start until Vercel env vars are present',
  PAYPAL_ORDER_CREATE_FAILED: 'PayPal order failed — provider order was not created',
  PAYPAL_SESSION_STORE_FAILED: 'PayPal session store failed — provider order was not safely recorded',
  SUPABASE_NOT_CONFIGURED: 'Supabase not configured — checkout cannot be stored safely',
  PAYMENT_TABLE_INSERT_FAILED: 'Payment table insert failed — checkout cannot be used until stored',
  AMOUNT_MISMATCH: 'Amount mismatch — received amount differs from expected deposit',
  CURRENCY_MISMATCH: 'Currency mismatch — received currency is not expected currency',
  CAPTURE_NOT_COMPLETED: 'Capture incomplete — PayPal capture was not completed',
};

function providerValue(row) {
  return String(rowValue(row, ['provider', 'payment_provider']) || 'XENDIT').toUpperCase();
}

function providerClasses(provider) {
  if (provider === 'PAYPAL') return 'border-blue-200 bg-blue-50 text-blue-700';
  if (provider === 'XENDIT') return 'border-violet-200 bg-violet-50 text-violet-700';
  return 'border-[#E5E0D8] bg-[#FBFAF7] text-[#6A645B]';
}
...
function ProviderBadge({ row }) {
  const provider = providerValue(row);
  return <span className={`inline-flex rounded-full border px-2.5 py-1 text-[10px] font-black uppercase tracking-widest ${providerClasses(provider)}`}>{provider}</span>;
}

function StatusHelp({ value }) {
  const help = PAYMENT_STATUS_LABELS[String(value || '').toUpperCase()];
  return help ? <span className="mt-1 block max-w-xs text-xs leading-snug text-[#6A645B]">{help}</span> : null;
}
...
function Reservation360Drawer({ row, onClose }) {
  ...
  const data = {
    ...
    payment: [['Provider', providerValue(row)], ['Provider session ID', rowValue(row, ['provider_session_id'])], ['Provider payment ID', rowValue(row, ['provider_payment_id'])], ['Provider reference ID', rowValue(row, ['provider_reference_id'])], ['Last webhook/event ID', rowValue(row, ['last_webhook_id', 'provider_event_id'])], ['Total amount', money(row.total_amount_php)], ['Deposit amount', money(row.deposit_amount_php)], ['Payment amount', money(row.payment_amount_php)], ['Balance amount', money(row.balance_amount_php)], ['Payment status', rowValue(row, ['payment_status'])], ['Booking payment status', rowValue(row, ['booking_payment_status'])], ['Verification status', rowValue(row, ['payment_verification_status'])], ['Verification note/error', rowValue(row, ['verification_error', 'verification_note', 'verification_notes'])]],
...
function ExceptionsTable({ rows }) {
  return <Panel title="Payment Exceptions" count={rows.length}><Table columns={['Reference', 'Provider', 'Issue', 'Expected', 'Received', 'Event', 'Notes']}>{rows.length === 0 ? <Empty colSpan={7} text="No payment exceptions." /> : rows.map((row) => <tr key={row.id || row.provider_event_id || JSON.stringify(row)} className="hover:bg-[#F4F0E8]/40"><td className="cell"><strong>{row.booking_reference || row.provider_reference_id || '—'}</strong></td><td className="cell"><ProviderBadge row={row} /></td><td className="cell"><Pill value={row.verification_status} /><StatusHelp value={row.verification_status} /></td><td className="cell">{money(row.expected_amount_php)} {row.expected_currency || 'PHP'}</td><td className="cell">{money(row.amount_php)} {row.currency || ''}</td><td className="cell">{row.event_type || '—'}<br /><span className="muted">{row.provider_event_id || ''}</span></td><td className="cell">{row.verification_notes || '—'}</td></tr>)}</Table></Panel>;
}
